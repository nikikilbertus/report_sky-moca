...
\end{equation}
%
\todo{want to design transition probabilities such that we reach equilibrium
boltzmann distribution, so everything is reversible and with the ratio we see
that transition probabilities only depend on differences in energy and
particularly not on the history of the chain, can always decide in the moment,
this is called detailed balance in metropolis hastings is one that fulfills it}

\todo{clarfiy somewhere that throughout all that sampling procedure~$T$ is fixed
at first.}
